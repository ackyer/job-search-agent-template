...
project. **This file is canonical**: `CLAUDE.md` and `GEMINI.md` only point here, so tools that
look for their own filename find the same rules. Edit rules here only.

If your tool reads none of these by default, tell it to read this file at the start of the
session.
